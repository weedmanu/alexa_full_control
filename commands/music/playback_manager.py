"""
Gestionnaire de lecture musicale avanc�e - API directe uniquement.

Utilise exclusivement l'API /api/np/command pour les contr�les musicaux,
sans fallback VoiceCommand. Plus rapide et direct que les commandes vocales.

Fonctionnalit�s:
- Contr�les de lecture (play, pause, next, prev, stop)
- Mode al�atoire (shuffle on/off)
- Mode r�p�tition (ONE, ALL - OFF non support� par API)
- Positionnement (seek)
- Historique de lecture

Note: Aucune fallback VoiceCommand - si l'API �choue, la commande �choue.
"""
...
class PlaybackManager(BaseManager[Dict[str, Any]]):
    ...
    Utilise exclusivement l'API /api/np/command pour tous les contr�les musicaux.
    Pas de fallback VoiceCommand - plus rapide mais moins tol�rant aux erreurs.
    """
    ...
    def __init__(
        ...
        # Initialiser VoiceCommandService pour les contr�les de lecture
        ...
        logger.info("PlaybackManager initialis� avec AlexaAPIService obligatoire et VoiceCommandService")

    def _send_np_command(self, command_data: Dict[str, Any], device_serial: str, device_type: str) -> bool:
        """Envoie une commande directe � /api/np/command (comme le script shell)."""
        try:
            with self._lock:
                ...
                logger.debug(f"Commande NP r�ussie: {command_data}")
                return True
        except Exception as e:
            command_type = command_data.get("type", "Unknown")
            # Pour certaines commandes (ShuffleCommand, RepeatCommand), 400 est normal
            # si elles ne sont pas support�es dans le contexte actuel
            if "400" in str(e) and command_type in ["ShuffleCommand", "RepeatCommand"]:
                logger.info(f"??  Commande {command_type} non support�e dans ce contexte (normal)")
                return False
            else:
                logger.warning(f"�chec commande NP {command_data}: {e}")
                return False
    ...
    def set_repeat(self, device_serial: str, device_type: str, mode: str) -> bool:
        """D�finit le mode r�p�tition (OFF, ONE, ALL)."""
        with self._lock:
            if not self.state_machine.can_execute_commands:
                return False
            ...
            # Utiliser l'API directe comme le script shell
            # Pour OFF, on n'envoie pas la commande (pas support� dans le script shell)
            if mode == "OFF":
                logger.warning("Mode OFF non support� par l'API directe")
                return False
    ...
    def seek_to(self, device_serial: str, device_type: str, position_ms: int) -> bool:
        """Se d�place � une position sp�cifique (en millisecondes)."""
        with self._lock:
            if not self.state_machine.can_execute_commands:
                return False
            try:
    ...
    def get_history(self, limit: int = 50) -> List[Dict[str, Any]]:
        """R�cup�re l'historique de lecture."""
        with self._lock:
            if not self.state_machine.can_execute_commands:
                return []
            try:
    ...
    def play_artist(self, device_serial: str, artist_name: str) -> bool:
        """Joue de la musique d'un artiste sp�cifique."""
        with self._lock:
            if not self.state_machine.can_execute_commands:
                return False
    ...
    def get_state(
        ...
        1. /api/np/player - �tat du player (morceau, artiste, progression)
        2. /api/media/state - �tat m�dia d�taill�
        3. /api/np/queue - File d'attente compl�te
        ...
        Returns:
            Dict avec les cl�s 'player', 'media', 'queue' ou None si erreur
        """
        with self._lock:
            if not self.state_machine.can_execute_commands:
                return None
            ...
            try:
                ...
                # 1. �tat du player (comme show_queue() du shell)
                logger.debug(f"R�cup�ration �tat player pour {device_serial}")
                player_data = self._api_service.get("/api/np/player", params=params)

                # 2. �tat m�dia
                logger.debug(f"R�cup�ration �tat m�dia pour {device_serial}")
                ...
                # 3. Queue compl�te
                logger.debug(f"R�cup�ration queue pour {device_serial}")
                queue_data = self._api_service.get("/api/np/queue", params=media_params)

                # Combiner les 3 r�ponses (comme le script shell)
                result: Dict[str, Any] = {"player": player_data, "media": media_data, "queue": queue_data}

                logger.success(f"�tat complet r�cup�r� pour {device_serial}")
                return result

            except Exception as e:
                logger.error(f"Erreur r�cup�ration �tat complet: {e}")
                logger.debug(f"D�tails erreur: {type(e).__name__}: {str(e)}")
                return None
