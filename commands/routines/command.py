...
class RoutineCommand(BaseCommand):
    ...
    def _list_routines(self, args: argparse.Namespace) -> bool:
        ...
        try:
            ...
            self.info("� R�cup�ration des routines...")
    ...
    def _show_info(self, args: argparse.Namespace) -> bool:
        """Afficher les d�tails d'une routine."""
        try:
            self.info(f"?? R�cup�ration d�tails routine '{args.name}'...")
    ...
    def _execute_routine(self, args: argparse.Namespace) -> bool:
        """Ex�cuter une routine."""
        try:
            self.info(f"??  Ex�cution routine '{args.name}'...")
            ...
            if result:
                self.success("? Routine ex�cut�e avec succ�s")
                return True
    ...
    def _disable_routine(self, args: argparse.Namespace) -> bool:
        """D�sactiver une routine."""
        try:
            self.info(f"? D�sactivation routine '{args.name}'...")
            ...
            if result:
                self.success("? Routine d�sactiv�e")
                return True
    ...
    def _display_routines(self, routines: List[Dict[str, Any]]) -> None:
        ...
        self.info(f"?? {len(routines)} routine(s) trouv�e(s):\n")
        ...
        # Afficher le tableau
        table = self.format_table(table_data, ["Nom", "�tat", "D�clencheur", "ID"])
        print(table)
    ...
    def _display_routine_details(self, routine: Dict[str, Any]) -> None:
        ...
        print(f"  �tat: {'Activ�e ?' if enabled else 'D�sactiv�e ?'}")
