"""
Contr�leur g�n�rique pour appareils Smart Home - Thread-safe.
"""
...
class SmartDeviceController(BaseManager[Dict[str, Any]]):
    """Contr�leur thread-safe pour appareils connect�s (serrures, volets, etc.).

    H�rite de `BaseManager` pour utiliser `self.http_client`, cache et locks.
    """
    ...
    def __init__(
        ...
        logger.info("SmartDeviceController initialis�")

    def get_smart_home_devices(self) -> List[Dict[str, Any]]:
        """
        R�cup�re tous les appareils Smart Home (lazy loading).

        Utilise le SyncService pour charger les donn�es � la demande.
        """
        with self._lock:
            if self._all_devices_cache:
                return self._all_devices_cache

            # Utiliser le cache directement (h�rit� de l'ancienne impl�mentation)
            ...
            logger.warning("Aucun cache smart_home_all trouv�")
            return []

    def get_all_locks(self) -> List[Dict[str, Any]]:
        """R�cup�re toutes les serrures depuis smart_home_all cache."""
        with self._lock:
    ...
    def get_all_plugs(self) -> List[Dict[str, Any]]:
        """R�cup�re toutes les prises depuis smart_home_all cache."""
        with self._lock:
    ...
    def set_percentage(self, entity_id: str, percentage: int) -> bool:
        """D�finit un pourcentage (volets, stores, etc.) 0-100."""
        with self._lock:
            if not self._check_connection():
                return False
    ...
    def get_device_state(self, entity_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.error(f"Erreur �tat appareil: {e}")
                return None
