"""
Contr�leur pour thermostats connect�s - Thread-safe.
"""
...
class ThermostatController(BaseManager[Dict[str, Any]]):
    """Contr�leur thread-safe pour thermostats connect�s.

    H�rite de `BaseManager` pour r�utiliser le cache et le client HTTP.
    """
    ...
    def __init__(
        ...
        logger.info("ThermostatController initialis�")

    def get_all_thermostats(self) -> list[dict[str, Any]]:
        """R�cup�re tous les thermostats depuis le cache smart_home_all."""
        with self._lock:
            # 1. Cache m�moire (TTL 5min)
            if self._thermostats_cache:
                logger.debug("Thermostats depuis cache m�moire")
            ...
            logger.warning("Aucun cache smart_home_all disponible, synchronisation n�cessaire")
            return []

    def _filter_thermostats(self, devices: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Filtre les thermostats depuis la liste compl�te smart devices."""
        thermostats = []
        for device in devices:
            ...
            # Filtre bas� sur categoryType ou deviceType
    ...
    def set_temperature(self, entity_id: str, target_celsius: float) -> bool:
        """D�finit la temp�rature cible en Celsius."""
        with self._lock:
            if not self._check_connection():
                return False
            if not 10.0 <= target_celsius <= 35.0:
                logger.error(f"Temp�rature invalide: {target_celsius}�C (10-35)")
                return False
            try:
                directive = {
                    "directive": {
                ...
                logger.success(f"Temp�rature {target_celsius}�C pour {entity_id}")
                return True
    ...
    def set_mode(self, entity_id: str, mode: str) -> bool:
        """D�finit le mode (HEAT, COOL, AUTO, ECO, OFF)."""
        with self._lock:
            if not self._check_connection():
                return False
    ...
    def adjust_temperature(self, entity_id: str, delta_celsius: float) -> bool:
        """Ajuste la temp�rature (+/- delta en Celsius)."""
        with self._lock:
            if not self._check_connection():
                return False
            try:
                ...
                logger.success(f"Temp�rature {sign}{delta_celsius}�C pour {entity_id}")
                return True
            except Exception as e:
                logger.error(f"Erreur ajustement temp�rature: {e}")
                return False
