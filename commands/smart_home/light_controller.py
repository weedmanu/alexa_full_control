...
class LightController(BaseManager[Dict[str, Any]]):
    ...
    def turn_on(self, entity_id: str, device_serial: Optional[str] = None) -> bool:
        ...
        if not light_name:
            logger.error(f"? Lumi�re introuvable: {entity_id}")
            return False
    ...
    def turn_off(self, entity_id: str, device_serial: Optional[str] = None) -> bool:
        """�teint lumi�re (accepte nom friendly ou entity_id)."""
        light_name = self._resolve_name(entity_id)
        if not light_name:
            logger.error(f"? Lumi�re introuvable: {entity_id}")
            return False
    ...
    def get_all_lights(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if (
                ...
                logger.debug("?? Lumi�res depuis cache m�moire")
    ...
    def _refresh_lights_cache(self) -> List[Dict[str, Any]]:
        """Rafra�chit le cache des lumi�res depuis l'API."""
        if not self._check_connection():
            return []
        ...
        if cached_all is not None:
            ...
            try:
                logger.debug("?? R�cup�ration smart devices depuis API (fallback)")
                response = self.breaker.call(
        ...
        logger.info(f"?? {len(lights)} lumi�re(s) filtr�e(s)")
        return lights
    ...
    def invalidate_lights_cache(self) -> None:
        ...
        with self._lock:
            ...
            logger.info("??? Cache lumi�res (m�moire) invalid�")
    ...
    def toggle(self, entity_id: str) -> bool:
        """
        Bascule l'�tat de la lumi�re (on ? off).
        ...
        with self._lock:
            ...
            try:
                ...
                if current_state is None:
                    logger.warning(f"�tat inconnu pour {entity_id}, allumage par d�faut")
                    return self.turn_on(entity_id)

                # Basculer
                if current_state == "ON":
                    logger.debug("�tat actuel: ON ? Extinction")
                    return self.turn_off(entity_id)
                else:
                    logger.debug("�tat actuel: OFF ? Allumage")
                    return self.turn_on(entity_id)
