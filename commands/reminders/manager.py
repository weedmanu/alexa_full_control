...
class ReminderManager(BaseManager[Dict[str, Any]]):
    ...
    def __init__(
        ...
        Args:
            ...
            api_service: Service API centralis� (OBLIGATOIRE en Phase 2)
        """
        ...
        logger.info("ReminderManager initialis� avec AlexaAPIService obligatoire")
    ...
    def get_reminders(self, device_serial: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if self._is_cache_valid():
                logger.debug("?? Cache rappels valide, utilisation du cache m�moire")
                ...
                # Si pas de cache disque ou expir�, rafra�chir depuis l'API
                if not cached_reminders:
                    cached_reminders = self._refresh_reminders_cache()
                else:
                    # Mettre � jour le cache m�moire et le timestamp
    ...
    def _refresh_reminders_cache(self) -> List[Dict[str, Any]]:
        """
        Rafra�chit le cache des rappels en effectuant un appel API.
        ...
        """
        try:
            logger.debug("?? R�cup�ration de tous les rappels depuis l'API notifications")
            ...
            logger.info(f"? {len(reminders)} rappel(s) r�cup�r�(s) et mis en cache (m�moire + disque)")
            return reminders
