...
class TimerManager(BaseManager[Dict[str, Any]]):
    ...
    def __init__(
        ...
        Args:
            ...
            state_machine: Machine � �tats pour gestion d'�tat
            api_service: Service API centralis� (MANDATORY, jamais None)
            cache_service: Service de cache optionnel (cr�� si None)
            cache_ttl: Dur�e de vie du cache en secondes (d�faut: 60)
        ...
        logger.info("? TimerManager initialis� avec api_service obligatoire")
    ...
    def _check_connection(self) -> bool:
        ...
        if not self.state_machine.can_execute_commands:
            logger.error(f"Impossible d'ex�cuter la commande - �tat: {self.state_machine.state.name}")
            return False
        return True
    ...
    def create_timer(
        ...
        """
        Cr�e un nouveau timer sur un appareil Alexa via AlexaAPIService.
        ...
        with self._lock:
            ...
            if not force_refresh and self._is_cache_valid():
                if self._timers_cache is not None:
                    logger.debug(f"? Cache m�moire: {len(self._timers_cache)} timer(s)")
    ...
    def _refresh_timers_cache(self) -> List[Dict[str, Any]]:
        """
        Rafra�chit le cache des timers via AlexaAPIService.
        ...
        """
        try:
            logger.debug("?? R�cup�ration de tous les timers depuis l'API notifications")

            # Phase 2: Use api_service directly (mandatory, no fallback)
            data = self._api_service.get("/api/notifications", timeout=10)

            if data is None:
                logger.warning("??  R�ponse vide pour les timers")
                timers = []
            else:
            ...
            logger.info(f"? {len(timers)} timer(s) actif(s) r�cup�r�(s) et mis en cache")
            return timers
    ...
    def cancel_all_timers(self, device_serial: str, device_type: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.success(f"{success_count}/{len(timers)} timer(s) annul�(s) pour {device_serial}")
