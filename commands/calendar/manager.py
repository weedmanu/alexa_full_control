...
class CalendarManager:
    ...
    def query_events(self, timeframe: str = "aujourd'hui", device_name: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            if success:
                logger.info(f"? Commande calendrier '{timeframe}' envoy�e � {device_name}")
                return f"Alexa va �noncer vos �v�nements {timeframe}"
            else:
                logger.error("? �chec de la commande calendrier")
                return None
    ...
    def get_events(self, limit: int = 50, days_ahead: int = 30) -> Optional[List[Dict[str, Any]]]:
        ...
        """
        logger.warning("?? API REST calendrier non disponible - utilisez query_events()")
    ...
    def add_event(
        ...
        """
        logger.warning("?? Cr�ation d'�v�nements non disponible via API Alexa")
        logger.info("?? Utilisez Google Calendar, Outlook ou Apple Calendar pour cr�er des �v�nements")
    ...
    def delete_event(self, event_id: str) -> bool:
        ...
        _ = event_id
        logger.warning("?? Suppression d'�v�nements non disponible via API Alexa")
        logger.info("?? Supprimez l'�v�nement depuis Google Calendar, Outlook ou Apple Calendar")
        return False
    ...
    def get_event_details(self, event_id: str) -> Optional[Dict[str, Any]]:
        ...
        _ = event_id
        logger.warning("?? D�tails d'�v�nements non disponibles via API Alexa")
        logger.info("?? Consultez l'�v�nement depuis Google Calendar, Outlook ou Apple Calendar")
        return None
