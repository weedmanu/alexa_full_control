...
class AlarmsCommands(TimeSubCommand):
    ...
    def create(self, args: argparse.Namespace) -> bool:
        ...
        try:
            ...
            self.info(f"? Cr�ation alarme{label_text} � {args.time} ({repeat_text}) sur '{args.device}'...")
            ...
            if result:
                ...
                self.success(f"? Alarme cr��e (ID: {alarm_id})")
                return True
    ...
    def delete(self, args: argparse.Namespace) -> bool:
        ...
        try:
            ...
            self.info(f"???  Suppression alarme {args.id} sur '{args.device}'...")
    ...
    def update(self, args: argparse.Namespace) -> bool:
        ...
        try:
            ...
            self.info(f"??  Modification alarme {args.id} sur '{args.device}'...")
    ...
    def disable(self, args: argparse.Namespace) -> bool:
        ...
        try:
            ...
            self.info(f"? D�sactivation alarme {args.id} sur '{args.device}'...")
            ...
            if result:
                self.success("? Alarme d�sactiv�e")
                return True
    ...
    def _parse_time(self, time_str: str) -> Optional[str]:
        ...
        Args:
            time_str: Cha�ne d'heure (ex: "07:30", "18:45")
    ...
    def setup_parsers(subparsers) -> None:
        ...
        create_parser.add_argument("--label", type=str, metavar="LABEL", help="�tiquette de l'alarme (optionnel)")
        create_parser.add_argument(
            "--recurring",
            type=str,
            metavar="DAYS",
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        """
        Configure le parser pour les commandes d'alarmes.

        Args:
            parser: Sous-parser pour la cat�gorie 'alarms'
        """
        from utils.cli.command_parser import UniversalHelpFormatter
        ...
        parser.description = "G�rer les alarmes Alexa"
    ...
    def execute(self, args: argparse.Namespace) -> bool:
        """
        Ex�cute la commande d'alarme selon l'action.
        ...
        if action == "create":
            return self.create_alarm(args)
        elif action == "list":
            return self.list_alarms(args)
        elif action == "delete":
            return self.delete_alarm(args)
        elif action == "update":
            return self.update_alarm(args)
        elif action == "enable":
            return self.enable_alarm(args)
        elif action == "disable":
            return self.disable_alarm(args)
        else:
            self.error(f"Action '{action}' non reconnue")
            return False

    def create_alarm(self, args: argparse.Namespace) -> bool:
        """Cr�er une alarme."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
    ...
    def list_alarms(self, args: argparse.Namespace) -> bool:
        """Lister les alarmes."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
    ...
    def delete_alarm(self, args: argparse.Namespace) -> bool:
        """Supprimer une alarme."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
    ...
    def update_alarm(self, args: argparse.Namespace) -> bool:
        """Modifier une alarme."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
    ...
    def enable_alarm(self, args: argparse.Namespace) -> bool:
        """Activer une alarme."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
    ...
    def disable_alarm(self, args: argparse.Namespace) -> bool:
        """D�sactiver une alarme."""
        try:
            serial = self.get_device_serial(args.device)
            if not serial:
