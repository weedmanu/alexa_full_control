...
function initAmazonProxy(_options, callbackCookie, callbackListening) {
        ...
        try {
            if (fs.existsSync(formerDataStorePath)) {
                ...
                if (typeof formerDataStore === 'object' && formerDataStore.storeVersion === FORMERDATA_STORE_VERSION) {
                    ...
                    _options.logger && _options.logger('Proxy Init : chargement des données temporaires comme secours pour les données précédentes');
    ...
    _options.logger && _options.logger('Proxy Init : réutilisation du frc des données précédentes');
    ...
    _options.logger && _options.logger('Proxy Init : réutilisation du map-md des données précédentes');
    ...
    _options.logger && _options.logger('Proxy Init : réutilisation du deviceId des données précédentes');
    ...
    _options.logger && _options.logger(`Routeur : ${url} / ${req.method} / ${JSON.stringify(req.headers)}`);
        if (req.headers.host === `${_options.proxyOwnIp}:${_options.proxyPort}`) {
            ...
            if (url === '/') { // initial redirect
                ...
                _options.logger && _options.logger(`Alexa-Cookie : Requête initiale : ${returnedInitUrl}`);
    ...
    _options.logger && _options.logger(`ERREUR : ${err}`);
        try {
            res.writeHead(500, {
                'Content-Type': 'text/plain'
            });
            res.end(`Erreur du proxy : ${err}`);
        } catch (err) {
            // ignore
        }
    }
    ...
    _options.logger && _options.logger(`Alexa-Cookie : Requête proxy : ${req.method} ${url}`);
    //_options.logger && _options.logger('Alexa-Cookie : Données de requête proxy : ' + customStringify(proxyReq, null, 2));

        if (typeof proxyReq.getHeader === 'function') {
            _options.logger && _options.logger(`Alexa-Cookie : En-têtes : ${JSON.stringify(proxyReq.getHeaders())}`);
        ...
        if (req.method === 'POST') {
            if (typeof proxyReq.getHeader === 'function' && proxyReq.getHeader('referer')) {
                ...
                if (fixedReferer ) {
                    ...
                    _options.logger && _options.logger(`Alexa-Cookie : Modification des en-têtes : Referer ajusté : ${fixedReferer}`);
                    modified = true;
                }
            }
            if (typeof proxyReq.getHeader === 'function' && proxyReq.getHeader('origin') !== `https://${proxyReq.getHeader('host')}`) {
                ...
                _options.logger && _options.logger('Alexa-Cookie : Modification des en-têtes : suppression de l’Origin');
                modified = true;
            }
    ...
    _options.proxyLogLevel === 'debug' && _options.logger && _options.logger(`Alexa-Cookie : Requête proxy : (modifiée:${modified})${customStringify(proxyReq, null, 2)}`);
    }
    ...
    _options.logger && _options.logger(`Alexa-Cookie : Réponse proxy depuis l’hôte : ${reqestHost}`);
    _options.proxyLogLevel === 'debug' && _options.logger && _options.logger(`Alexa-Cookie : En-têtes de réponse proxy : ${customStringify(proxyRes.headers, null, 2)}`);
    _options.proxyLogLevel === 'debug' && _options.logger && _options.logger(`Alexa-Cookie : Flux sortant de réponse proxy : ${customStringify(proxyRes.socket.parser.outgoing, null, 2)}`);
    ...
    _options.logger && _options.logger(`Alexa-Cookie : Cookies traités : ${JSON.stringify(proxyCookies)}`);
        ...
        if (
            ...
            _options.logger && _options.logger('Alexa-Cookie : Connexion via proxy réussie !');
            ...
            _options.logger && _options.logger(`Alexa-Cookie : Proxy - cookies capturés : ${proxyCookies}`);
            _options.logger && _options.logger(`Alexa-Cookie : Proxy - paramètres capturés : ${JSON.stringify(queryParams)}`);
    ...
    // Si une redirection est détectée, réécrire l’en-tête Location
        if (proxyRes.headers.location) {
            _options.logger && _options.logger(`Redirection : destination initiale ----> ${proxyRes.headers.location}`);
            ...
            _options.logger && _options.logger(`Redirection : destination finale ----> ${proxyRes.headers.location}`);
            return;
        }
        ...
        modifyResponse(res, (proxyRes && proxyRes.headers ? proxyRes.headers['content-encoding'] || '' : ''), function(body) {
            if (body) {
                ...
                if (body !== bodyOrig) {
                    _options.logger && _options.logger('Alexa-Cookie : Corps de réponse modifié pour réécrire les URL');
    ...
    if (_options.proxyPort< 1 || _options.proxyPort > 65535) {
        _options.logger && _options.logger(`Alexa-Cookie : Erreur : Port ${_options.proxyPort} invalide. Utilisation d'un port aléatoire.`);
        _options.proxyPort = undefined;
    }
    const server = app.listen(_options.proxyPort, _options.proxyListenBind, function() {
        _options.logger && _options.logger(`Alexa-Cookie : Serveur proxy en écoute sur le port ${server.address().port}`);
        callbackListening && callbackListening(server);
        callbackListening = null;
    }).on('error', err => {
        _options.logger && _options.logger(`Alexa-Cookie : Erreur du serveur proxy : ${err}`);
