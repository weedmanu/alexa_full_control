/**
 * Basé en partie sur Amazon Alexa Remote Control (PLAIN shell)
 * http://blog.loetzimmer.de/2017/10/amazon-alexa-hort-auf-die-shell-echo.html ET sur
 * https://github.com/thorsten-gehrig/alexa-remote-control
 * et beaucoup amélioré ...
 *
 * Bibliothèque pour récupérer des cookies Alexa pour utilisation avec alexa_remote_control.
 */
...
const amazonProxy = require('./utils/proxy.js');
const {
    DEFAULT_AMAZON_PAGE,
    DEFAULT_USER_AGENT,
...
const defaultAmazonPage = DEFAULT_AMAZON_PAGE;
const defaultUserAgent = DEFAULT_USER_AGENT;
const defaultUserAgentLinux = DEFAULT_USER_AGENT_LINUX;
const defaultProxyCloseWindowHTML = DEFAULT_PROXY_CLOSE_WINDOW_HTML;
const defaultAcceptLanguage = DEFAULT_ACCEPT_LANGUAGE;
const apiCallVersion = API_CALL_VERSION;
const apiCallUserAgent = API_CALL_USER_AGENT;
const defaultAppName = DEFAULT_APP_NAME;
const csrfOptions = CSRF_OPTIONS;
...
function AlexaCookie() {
        ...
        for (let cookie of headers['set-cookie']) {
            ...
            if (cookie && cookie.length === 3) {
                ...
                if (cookies[cookie[1]] && cookies[cookie[1]] !== cookie[2]) {
                    _options.logger && _options.logger(`Alexa-Cookie : Mise à jour du cookie ${cookie[1]} = ${cookie[2]}`);
                } else if (!cookies[cookie[1]]) {
                    _options.logger && _options.logger(`Alexa-Cookie : Ajout du cookie ${cookie[1]} = ${cookie[2]}`);
                }
    ...
    const request = (options, info, callback) => {
        _options.logger && _options.logger(`Alexa-Cookie : Envoi de la requête avec ${JSON.stringify(options)}`);
        ...
        const req = https.request(options, (res) => {
            ...
            if (options.followRedirects !== false && res.statusCode >= 300 && res.statusCode < 400) {
                _options.logger && _options.logger(`Alexa-Cookie : Réponse (${res.statusCode})${res.headers.location ? ` - Redirection vers ${res.headers.location}` : ''}`);
                ...
                _options.logger && _options.logger(`Alexa-Cookie : Réponse (${res.statusCode})`);
                res.on('data', (chunk) => {
                    body += chunk;
                });
    ...
    const initConfig = () => {
        ...
        _options.logger && _options.logger(`Alexa-Cookie : Utilisé comme URL de connexion Amazon : ${_options.amazonPage}`);

        _options.baseAmazonPage = _options.baseAmazonPage || 'amazon.com';
    _options.logger && _options.logger(`Alexa-Cookie : URL Amazon de base utilisée : ${_options.baseAmazonPage}`);

        _options.deviceAppName = _options.deviceAppName || defaultAppName;
    _options.logger && _options.logger(`Alexa-Cookie : Nom d'application de l'appareil utilisé : ${_options.deviceAppName}`);
    ...
    _options.logger && _options.logger(`Alexa-Cookie : User-Agent utilisé : ${_options.userAgent}`);

        _options.acceptLanguage = _options.acceptLanguage || defaultAcceptLanguage;

    _options.logger && _options.logger(`Alexa-Cookie : En-tête Accept-Language utilisé : ${_options.acceptLanguage}`);
        ...
        if (_options.setupProxy && !_options.proxyOwnIp) {
            _options.logger && _options.logger('Alexa-Cookie : Paramètre proxyOwnIp manquant pour le proxy. Désactivation !');
        ...
        if (_options.setupProxy) {
            ...
            _options.logger && _options.logger(`Alexa-Cookie : Mode proxy activé si nécessaire : ${_options.proxyOwnIp}:${_options.proxyPort} en écoute sur ${_options.proxyListenBind}`);
        } else {
            _options.setupProxy = false;
            _options.logger && _options.logger('Alexa-Cookie : Mode proxy désactivé');
        }
    ...
    const getCSRFFromCookies = (cookie, _options, callback) => {
        ...
        function csrfTry() {
            ...
            _options.logger && _options.logger(`Alexa-Cookie : Étape 4 : récupération du CSRF via ${path}`);
            request(options, (error, response) => {
                cookie = addCookies(cookie, response ? response.headers : null);
                const ar = /csrf=([^;]+)/.exec(cookie);
                const csrf = ar ? ar[1] : undefined;
                _options.logger && _options.logger(`Alexa-Cookie : Résultat : csrf=${csrf}, Cookie=${cookie}`);
    ...
    this.generateAlexaCookie = (email, password, __options, callback) => {
        ...
        if (!_options.proxyOnly) {
            ...
            _options.logger && _options.logger('Alexa-Cookie : Étape 1 : obtenir le premier cookie et la redirection d’authentification');
            request(options, (error, response, body, info) => {
                ...
                _options.logger && _options.logger('Alexa-Cookie : Étape 2 : connexion vide pour générer la session');
                request(options, (error, response, body) => {
                    ...
                    _options.logger && _options.logger('Alexa-Cookie : Étape 3 : connexion avec formulaire rempli, le referer contient l’identifiant de session');
                    request(options, (error, response, body, info) => {
                        if (error) {
                            callback && callback(error, null);
                            return;
                        ...
                        if (!lastRequestOptions.host.startsWith('alexa') || !lastRequestOptions.path.endsWith('.html')) {
                            ...
                            if (_options.setupProxy) {
                                if (proxyServer) {
                                    errMessage += ` Merci d’ouvrir http://${_options.proxyOwnIp}:${_options.proxyPort}/ dans votre navigateur et de vous connecter à Amazon. Le cookie s’affichera ici après une connexion réussie.`;
                                } else {
            ...
            amazonProxy.initAmazonProxy(_options, prepareResult, (server) => {
                ...
                const errMessage = `Merci d’ouvrir http://${_options.proxyOwnIp}:${_options.proxyPort}/ dans votre navigateur et de vous connecter à Amazon. Le cookie s’affichera ici après une connexion réussie.`;
                callback && callback(new Error(errMessage), null);
            });
        }
    ...
    this.construireConfigAlexaRemote = (registrationData) => {
        if (!registrationData || typeof registrationData !== 'object') {
            throw new Error('Aucune donnée d’enregistrement fournie pour construire la configuration Alexa Remote.');
        }

        const cookie = registrationData.localCookie || registrationData.cookie || registrationData.loginCookie;
        if (!cookie) {
            throw new Error('Impossible de déterminer un cookie Alexa valide à partir des données fournies.');
        }

        return {
            cookie,
            csrf: registrationData.csrf,
            macDms: registrationData.macDms
        };
    };

    const handleTokenRegistration = (_options, loginData, callback) => {
    _options.logger && _options.logger(`Alexa-Cookie : Début de l’enregistrement des jetons : ${JSON.stringify(loginData)}`);
    ...
    _options.logger && _options.logger('Alexa-Cookie : Enregistrement de l’application');
        ...
        request(options, (error, response, body) => {
            ...
            try {
                ...
                _options.logger && _options.logger(`Réponse de l’enregistrement d’application : ${JSON.stringify(body)}`);
            ...
            registerTokenCapabilities(accessToken, () => {
                ...
                */
                ...
                _options.logger && _options.logger('Alexa-Cookie : Récupération des données utilisateur');
                _options.logger && _options.logger(JSON.stringify(options));
                request(options, (error, response, body) => {
                    if (!error) {
                        try {
    ...
    _options.logger && _options.logger('Alexa-Cookie : Enregistrement des capacités');
        _options.logger && _options.logger(JSON.stringify(options));
        request(options, (error, response, body) => {
            if (error || (response.statusCode !== 204 && response.statusCode !== 200)) {
                _options.logger && _options.logger('Alexa-Cookie : Impossible de définir les capacités, la connexion Push pourrait ne pas fonctionner !');
    ...
    _options.logger && _options.logger(`Alexa-Cookie : Échange des jetons pour ${amazonPage}`);
        _options.logger && _options.logger(JSON.stringify(options));
        ...
        request(options, (error, response, body) => {
            ...
            body.response.tokens.cookies[`.${amazonPage}`].forEach((cookie) => {
                if (cookies[cookie.Name] && cookies[cookie.Name] !== cookie.Value) {
                    _options.logger && _options.logger(`Alexa-Cookie : Mise à jour du cookie ${cookie.Name} = ${cookie.Value}`);
                } else if (!cookies[cookie.Name]) {
                    _options.logger && _options.logger(`Alexa-Cookie : Ajout du cookie ${cookie.Name} = ${cookie.Value}`);
                }
    ...
    this.refreshAlexaCookie = (__options, callback) => {
        ...
        Cookie = _options.formerRegistrationData.loginCookie;
    _options.logger && _options.logger('Alexa-Cookie : Rafraîchissement du jeton');
        _options.logger && _options.logger(JSON.stringify(options));
        request(options, (error, response, body) => {
            if (error) {
                callback && callback(error, null);
