...
class AuthCommand(BaseCommand):
        ...
        try:
            ...
            if force:
                ...
                if self._has_valid_cookies():
                    ...
                    print("\033[1;33m??  Session d'authentification déjà active\033[0m")
                    print()
    ...
    def _status(self, args: argparse.Namespace) -> bool:
        ...
        """
        self.logger.info("??  État de connexion")
        print()
    ...
    def _delete_existing_cookies(self) -> None:
        ...
        for file_path in files_to_delete:
            if file_path.exists():
                file_path.unlink()
                self.logger.debug(f"??? Cookie supprimé: {file_path}")
                deleted_count += 1

        if deleted_count > 0:
            self.logger.info(f"??? {deleted_count} cookie(s) existant(s) supprimé(s)")
        else:
    ...
    def _show_installation_instructions(self, issues):
        ...
        self.logger.error("?? Prérequis manquants détectés")
        self.logger.info("")
