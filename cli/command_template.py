        ...
        try:
            ...
            if not self.validate(params):
                raise ValidationError(
                    f"Invalid parameters for {self.command_name}"
                )
            ...
            self.logger.error(
                f"Unexpected error executing {self.command_name}: {e}",
                exc_info=True
            )
            return {
                "success": False,
...
class ManagerCommandBuilder:
    ...
    def with_di_container(
        self, di_container: DIContainer
    ) -> "ManagerCommandBuilder":
...
class CommandRegistry:
    ...
    def register(
        self, command_name: str, command: ManagerCommand
    ) -> None:
        """
        Register a command.
        ...
        if command_name in self._commands:
            self._logger.warning(
                f"Overwriting existing command: {command_name}"
            )
