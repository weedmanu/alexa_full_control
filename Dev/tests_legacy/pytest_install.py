        ...
        if isinstance(node, ast.FunctionDef) or isinstance(node, ast.AsyncFunctionDef):
            ...
            for dec in node.decorator_list:
                if (
                    ...
                    and getattr(dec.attr, "__str__", None) is not None
                ):
                    # generic
                    pass
...
def test_check_disk_space(monkeypatch, tmp_path):
    ...
    # Enough space
    monkeypatch.setattr(
        install_mod.shutil, "disk_usage", lambda p: SimpleNamespace(free=5 * 1024**3)
    )
    ...
    # Low space
    monkeypatch.setattr(
        install_mod.shutil, "disk_usage", lambda p: SimpleNamespace(free=0.5 * 1024**3)
    )
...
def test_install_python_packages_with_requirements(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        install_mod.PackageInstaller, "run_command", lambda self, *a, **k: fake_run(*a, **k)
    )
    inst = install_mod.PackageInstaller(tmp_path)
...
def test_install_npm_packages_node_unavailable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        install_mod.PackageInstaller, "run_command", lambda self, *a, **k: fake_run_node(*a, **k)
    )
    inst = install_mod.PackageInstaller(tmp_path)
    try:
...
def test_cleanup_existing_installation_force(monkeypatch, tmp_path):
    ...
    removed = []
    monkeypatch.setattr(
        install_mod.shutil, "rmtree", lambda p, ignore_errors=True: removed.append(str(p))
    )
...
def test_run_system_checks_calls_systemchecker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        install_mod.SystemChecker, "check_python_version", staticmethod(lambda: (False, "bad"))
    )
    monkeypatch.setattr(
        install_mod.sys, "exit", lambda code=0: (_ for _ in ()).throw(SystemExit(code))
    )
...
def test_run_installation_full_flow(monkeypatch, tmp_path):
    ...
    calls = []
    monkeypatch.setattr(
        install_mod.PackageInstaller, "create_venv", lambda self: calls.append("create_venv")
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller, "upgrade_pip", lambda self: calls.append("upgrade_pip")
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "install_python_packages",
        lambda self: calls.append("install_python_packages"),
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller, "install_nodejs", lambda self: calls.append("install_nodejs")
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "install_npm_packages",
        lambda self: calls.append("install_npm_packages"),
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "create_data_directory",
        lambda self: calls.append("create_data_directory"),
    )
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "test_configuration",
        lambda self: calls.append("test_configuration"),
    )

    # Ensure check_existing_installation returns False to run full installation
    monkeypatch.setattr(
        install_mod.InstallationManager, "check_existing_installation", lambda self: False
    )
...
def test_main_normal_flow_calls_manager(monkeypatch, tmp_path):
    ...
    assert (
        called.get("init")
        and called.get("checks")
        and called.get("install")
        and called.get("summary")
    )
...
def test_install_nodejs_makes_nodejs_dir_and_calls_nodeenv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "get_venv_python",
        lambda self: tmp_path / ".venv" / "bin" / "python",
    )
...
def test_test_configuration_python_and_node(monkeypatch, tmp_path, capsys):
    ...
    node_path = tmp_path / "node"
    monkeypatch.setattr(
        install_mod.PackageInstaller, "get_nodejs_paths", lambda self: (node_path, tmp_path / "npm")
    )
    ...
    monkeypatch.setattr(
        install_mod.PackageInstaller,
        "run_command",
        lambda self, cmd, cwd=None, capture_output=False, **kwargs: fake_run(
            cmd, cwd=cwd, capture_output=capture_output, **kwargs
        ),
    )
    # Should not raise
...
def test_run_installation_existing_install_calls_cleanup(monkeypatch, tmp_path):
    ...
    mgr = install_mod.InstallationManager(d)
    monkeypatch.setattr(
        install_mod.InstallationManager, "check_existing_installation", lambda self: True
    )
    called = {}
    monkeypatch.setattr(
        install_mod.InstallationManager,
        "cleanup_existing_installation",
        lambda self: called.setdefault("cleanup", True),
    )
...
def test_run_system_checks_branches(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        install_mod.SystemChecker, "check_disk_space", lambda path, required_gb=2.0: (False, "low")
    )
    mgr.run_system_checks()
...
def test_wrap_and_strip_edge_emojis_small_width(capsys):
    ...
    install_mod.Logger._wrap_and_print(
        "ℹ️", "🎉" + "A" * 100 + "🎉", install_mod.Colors.CYAN, max_width=10
    )
    out = capsys.readouterr().out
    assert "A" in out
...
def test_run_system_checks_disk_branches(monkeypatch, tmp_path):
    ...
    # disk ok
    monkeypatch.setattr(
        install_mod.SystemChecker,
        "check_disk_space",
        lambda path, required_gb=2.0: (True, "10.0 GB disponibles"),
    )
    mgr.run_system_checks()
    ...
    monkeypatch.setattr(
        install_mod.SystemChecker,
        "check_disk_space",
        lambda path, required_gb=2.0: (False, "0.5 GB disponibles"),
    )
    mgr.run_system_checks()
...
def test_core_main_blocks_uninstall_when_running_in_venv():
    ...
    args = types.SimpleNamespace(
        uninstall=True, force=False, skip_tests=False, dry_run=False, yes=False
    )
...
def test_logger_internal_wrap_strips_edge_emojis_small_width(capsys):
    ...
    install_mod.Logger._internal_wrap_and_print(
        "✅", "🎉 Hello World 🎉", install_mod.Colors.GREEN, max_width=10
    )
    out = capsys.readouterr().out
